    ...
    public function myCollect(Request $request): JsonResponse|array
    {
        if(isset($request->params)){
            ...
            $paginator = DB::table('view_record')
                ...
                ->select(
                    'video.id','video.name','video.gold','video.cat','video.sync','video.title','video.duration','video.type','video.restricted','video.cover_img','video.views','video.updated_at','video.hls_url','video.dash_url','video.comments','video.likes',
            ...
            //路径处理
            $res['list'] = $this->handleVideoItems($paginator->toArray()['data'] ?? [],true, true);
            //时长转秒
